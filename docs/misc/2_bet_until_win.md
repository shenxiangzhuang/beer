...
??? success "Solution1: Analysis"
    ...
    === "中文"
        ...
        首先让我们稍微简化一下问题：整个赌局从开始到结束，总共就是这11块钱。
        结束的标志的就是其中一个人的钱变为0：要么A输光，此时B的钱是11块；要么B输光，此时A的钱是11块。

        设事件即A拥有$x$块钱时获胜的概率为$P(A=x) = p_{x}$，那么根据游戏规则有：
